...
//
// On Wed Sep  9 12:58:03 CEST 2020
//
//
// Ports:
// Name                         I/O  size props
...
module mkRVController(CLK,
  ...
  wire [63 : 0] x__h4178, x__h4264;
  wire [31 : 0] x_data__h3669;

  // value method rv_rstn
  assign rv_rstn = state != 2'd0 && state != 2'd1 ;

  // value method rv_reset
  assign rv_reset = state == 2'd0 || state == 2'd1 ;

  // value method tapasco_intr
  assign tapasco_intr = WILL_FIRE_RL_graceDown && grace == 7'd0 ;
  ...
  assign WILL_FIRE_RL_cycle_count =
	     state != 2'd0 && !WILL_FIRE_RL_handle_start ;

  // rule RL_endReset
  assign WILL_FIRE_RL_endReset = state == 2'd1 && !WILL_FIRE_RL_handle_start ;

  // rule RL_bsvreset
  assign WILL_FIRE_RL_bsvreset =
	     state == 2'd2 && resetBSV && reqRDY_req_rdy &&
	     !WILL_FIRE_RL_handle_start ;

  // rule RL_startGracePeriod
  assign WILL_FIRE_RL_startGracePeriod =
	     state == 2'd2 && intrWire$whas && !WILL_FIRE_RL_handle_start ;

  // rule RL_graceDown
  assign WILL_FIRE_RL_graceDown =
	     state == 2'd3 && !WILL_FIRE_RL_handle_start ;
  ...
  // register grace
  assign grace$D_IN =
	     WILL_FIRE_RL_graceDown ? MUX_grace$write_1__VAL_1 : 7'd120 ;
  ...
  // register ret
  assign ret$D_IN = (wr_s_in$D_OUT[54:39] == 16'h0010) ? x__h4178 : x__h4264 ;
  assign ret$EN =
	     WILL_FIRE_RL_handle_write && wr_s_in$D_OUT[6:3] != 4'd0 &&
  ...
  // register state
  always@(tapasco_intr or
	  WILL_FIRE_RL_handle_start or
	  WILL_FIRE_RL_endReset or WILL_FIRE_RL_startGracePeriod)
  begin
    case (1'b1) // synopsys parallel_case
      tapasco_intr: state$D_IN = 2'd0;
      WILL_FIRE_RL_handle_start: state$D_IN = 2'd1;
      WILL_FIRE_RL_endReset: state$D_IN = 2'd2;
      WILL_FIRE_RL_startGracePeriod: state$D_IN = 2'd3;
  ...
  // register stdin_base
  assign stdin_base$D_IN = tapasco_intr ? 32'd0 : wr_s_in$D_OUT[38:7] ;
  assign stdin_base$EN =
	     WILL_FIRE_RL_graceDown && grace == 7'd0 ||
	     state == 2'd0 && stdinBaseWire$whas ;

  // register stdin_size
  assign stdin_size$D_IN = tapasco_intr ? 32'd0 : wr_s_in$D_OUT[38:7] ;
  assign stdin_size$EN =
	     WILL_FIRE_RL_graceDown && grace == 7'd0 ||
	     state == 2'd0 && stdinSizeWire$whas ;

  // register stdout_base
  assign stdout_base$D_IN = tapasco_intr ? 32'd0 : wr_s_in$D_OUT[38:7] ;
  assign stdout_base$EN =
	     WILL_FIRE_RL_graceDown && grace == 7'd0 ||
	     state == 2'd0 && stdoutBaseWire$whas ;

  // register stdout_size
  assign stdout_size$D_IN = tapasco_intr ? 32'd0 : wr_s_in$D_OUT[38:7] ;
  assign stdout_size$EN =
	     WILL_FIRE_RL_graceDown && grace == 7'd0 ||
	     state == 2'd0 && stdoutSizeWire$whas ;
  ...
  begin
    case (rd_s_in$D_OUT[10:3])
      8'h0: rd_s_out$D_IN = { x_data__h3669, 2'd0 };
  ...
  // remaining internal signals
  assign x__h4178 = { ret[63:32], wr_s_in$D_OUT[38:7] } ;
  assign x__h4264 = { wr_s_in$D_OUT[38:7], ret[31:0] } ;
  assign x_data__h3669 = (state == 2'd0) ? 32'd0 : 32'd1 ;
