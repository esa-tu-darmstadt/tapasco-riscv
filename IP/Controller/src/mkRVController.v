...
//
// On Thu Sep 24 15:33:36 CEST 2020
//
//
// Ports:
// Name                         I/O  size props
...
module mkRVController(CLK,
  ...
  // value method rv_rstn
  assign rv_rstn = state != 3'd0 && state != 3'd1 ;

  // value method rv_reset
  assign rv_reset = state == 3'd0 || state == 3'd1 ;

  // value method tapasco_intr
  assign tapasco_intr = state == 3'd4 ;
  ...
  assign WILL_FIRE_RL_writeStdoutBase = state == 3'd0 && stdoutBaseWire$whas ;

  // rule RL_writeStdoutSize
  assign WILL_FIRE_RL_writeStdoutSize = state == 3'd0 && stdoutSizeWire$whas ;

  // rule RL_writeStdinBase
  assign WILL_FIRE_RL_writeStdinBase = state == 3'd0 && stdinBaseWire$whas ;

  // rule RL_writeStdinSize
  assign WILL_FIRE_RL_writeStdinSize = state == 3'd0 && stdinSizeWire$whas ;

  // rule RL_handle_start
  assign WILL_FIRE_RL_handle_start =
	     WILL_FIRE_RL_handle_write && wr_s_in$D_OUT[6:3] != 4'd0 &&
	     wr_s_in$D_OUT[54:39] == 16'h0 &&
	     wr_s_in$D_OUT[38:7] != 32'd0 ;

  // rule RL_cycle_count
  assign WILL_FIRE_RL_cycle_count =
	     state != 3'd0 && !WILL_FIRE_RL_handle_start ;

  // rule RL_endReset
  assign WILL_FIRE_RL_endReset = state == 3'd1 && !WILL_FIRE_RL_handle_start ;

  // rule RL_bsvreset
  assign WILL_FIRE_RL_bsvreset =
	     state == 3'd2 && resetBSV && reqRDY_req_rdy &&
	     !WILL_FIRE_RL_handle_start ;

  // rule RL_startGracePeriod
  assign WILL_FIRE_RL_startGracePeriod =
	     state == 3'd2 && intrWire$whas && !WILL_FIRE_RL_handle_start ;

  // rule RL_graceDown
  assign WILL_FIRE_RL_graceDown =
	     state == 3'd3 && !WILL_FIRE_RL_handle_start ;

  // rule RL_interrupt
  assign WILL_FIRE_RL_interrupt =
	     state == 3'd4 && !WILL_FIRE_RL_handle_start ;

  // inputs to muxes for submodule ports
  assign MUX_state$write_1__SEL_1 = WILL_FIRE_RL_graceDown && grace == 7'd0 ;
  ...
  // register counter
  assign counter$D_IN =
	     WILL_FIRE_RL_cycle_count ? MUX_counter$write_1__VAL_1 : 64'd0 ;
  assign counter$EN = WILL_FIRE_RL_cycle_count || WILL_FIRE_RL_handle_start ;

  // register grace
  assign grace$D_IN =
	     WILL_FIRE_RL_graceDown ? MUX_grace$write_1__VAL_1 : 7'd124 ;
  assign grace$EN = WILL_FIRE_RL_graceDown || WILL_FIRE_RL_startGracePeriod ;
  ...
  always@(MUX_state$write_1__SEL_1 or
	  WILL_FIRE_RL_interrupt or
	  WILL_FIRE_RL_handle_start or
	  WILL_FIRE_RL_endReset or WILL_FIRE_RL_startGracePeriod)
  begin
    case (1'b1) // synopsys parallel_case
      MUX_state$write_1__SEL_1: state$D_IN = 3'd4;
      WILL_FIRE_RL_interrupt: state$D_IN = 3'd0;
      WILL_FIRE_RL_handle_start: state$D_IN = 3'd1;
      WILL_FIRE_RL_endReset: state$D_IN = 3'd2;
      WILL_FIRE_RL_startGracePeriod: state$D_IN = 3'd3;
      default: state$D_IN = 3'b010 /* unspecified value */ ;
    endcase
  end
  assign state$EN =
	     WILL_FIRE_RL_graceDown && grace == 7'd0 ||
	     WILL_FIRE_RL_interrupt ||
	     WILL_FIRE_RL_handle_start ||
	     WILL_FIRE_RL_endReset ||
	     WILL_FIRE_RL_startGracePeriod ;
  ...
  assign stdin_base$EN =
	     WILL_FIRE_RL_writeStdinBase || WILL_FIRE_RL_interrupt ;

  // register stdin_size
  assign stdin_size$D_IN =
	     WILL_FIRE_RL_writeStdinSize ? wr_s_in$D_OUT[38:7] : 32'd0 ;
  assign stdin_size$EN =
	     WILL_FIRE_RL_writeStdinSize || WILL_FIRE_RL_interrupt ;

  // register stdout_base
  assign stdout_base$D_IN =
	     WILL_FIRE_RL_writeStdoutBase ? wr_s_in$D_OUT[38:7] : 32'd0 ;
  assign stdout_base$EN =
	     WILL_FIRE_RL_writeStdoutBase || WILL_FIRE_RL_interrupt ;

  // register stdout_size
  assign stdout_size$D_IN =
	     WILL_FIRE_RL_writeStdoutSize ? wr_s_in$D_OUT[38:7] : 32'd0 ;
  assign stdout_size$EN =
	     WILL_FIRE_RL_writeStdoutSize || WILL_FIRE_RL_interrupt ;
  ...
  assign x_data__h3631 = (state == 3'd0) ? 32'd0 : 32'd1 ;
