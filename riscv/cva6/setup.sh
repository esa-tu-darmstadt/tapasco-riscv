...
#sv2v -D SYNTHESIS -D VERILATOR -I "include" -I "src/common_cells/include" -I "src/common_cells/include/common_cells" --exclude=interface -w cva6_top.v `cat ../core.files`
# Dirty hack to remove a type cast that vivado does not like...
#sed -i "s/int unsigned'//g" cva6_top.v
#sv2v -D SYNTHESIS -D VERILATOR -I "include" -I "src/common_cells/include" -I "src/common_cells/include/common_cells" --exclude=interface -w dm_top.v `cat ../dm_core.files`
#sv2v -D SYNTHESIS -D VERILATOR -I "include" -I "src/common_cells/include" -I "src/common_cells/include/common_cells" --exclude=interface -w clint_top.v `cat ../clint_core.files`
